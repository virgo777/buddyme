...
>
> 推荐阅读：[ReAct、Plan-and-Execute 与 Reflection 的本质差异与落地指南](http://49.235.53.176/blog/react-plan-and-execute-reflection) — 深入理解 buddyMe 任务拆解背后的 Agent 设计范式。
...
用户输入
  ...
  └─ 普通输入
      ...
      └─ 复杂任务
          │
          ├─ 阶段一：任务规划 ──→ LLM 拆分为子任务列表
          │
          ├─ 阶段二：子任务执行 ──→ 逐个执行，每个子任务独立 LLM+工具循环
          │                       技能注入 · 结果传递 · 类型分类（research/build/verify）
          │
